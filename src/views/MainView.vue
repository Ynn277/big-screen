  ...
  <div class="main-layout">
    ...
    <div class="side-column left-col">
      ...
      <div class="sub-panel overview-panel">
        <div class="panel-title">| {{ isOverview ? '全国总览' : currentData.name }}</div>
...
let pieChart = null, lineChart = null, radarChart = null
...
const updateCharts = () => {
  ...
  if (pieChart) {
    ...
    // 或者更接近你截图的配色：
    const targetColors = ['#4593fc', '#fddb5a', '#54d8b9', '#9be564', '#f59056'];
    pieChart.setOption({
  ...
  if (lineChart) {
    lineChart.setOption({
      ...
      yAxis: { type: 'value', splitLine: { show: false }, axisLabel: { color: '#b0cbe9',textBorderColor: '#000000',
    textBorderWidth: 2,
  ...
  if (radarChart) {
    radarChart.setOption({
      ...
      radar: {
        ...
        name: { textStyle: { color: '#b0cbe9',textBorderColor: '#000000',
      textBorderWidth: 2,
      textShadowColor: '#000000',
      textShadowBlur: 5 } }
      },
...
.stat-row { margin-bottom: 10px; padding-left: 10px; color: #b0cbe9; font-size: 14px; }
.stat-row .value { color: #fff; font-weight: bold; margin-left: 5px; }

.stat-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; padding: 0 5px; }
...
.project-row.active-row { 
  background: linear-gradient(90deg, rgba(0, 228, 255, 0.3) 0%, rgba(0, 228, 255, 0) 100%); /* 青色渐变背景 */
...
.progress-track { height: 3px; background: rgba(255,255,255,0.1); border-radius: 2px; }
.progress-bar { height: 100%; border-radius: 2px; transition: all 0.5s; }

/* 右侧：图表容器 */
.top-panel, .mid-panel, .bot-panel { flex: 1; min-height: 0; display: flex; flex-direction: column; }
.chart-box { flex: 1; width: 100%; min-height: 0; }

.fault-summary { display: flex; justify-content: space-around; margin-bottom: 5px; }
.f-box { text-align: center; font-size: 13px; color: #b0cbe9; }
.num-warn { display: block; font-size: 20px; color: #ffba00; font-weight: bold; margin-top: 2px; }
.num-safe { display: block; font-size: 20px; color: #00ff72; font-weight: bold; margin-top: 2px; }
.glow-effect {
  position: absolute;
